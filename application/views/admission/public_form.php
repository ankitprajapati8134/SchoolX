...
function simulateMockPayment(order) {
    ...
    setTimeout(function() {
        // Mock: generate a fake gateway payment ID and signature
        var mockPid = 'pay_MOCK_' + Date.now();
        var mockSig = 'mock_verified'; // The mock gateway accepts this pattern

        // Actually call the mock gateway's simulate endpoint via server
        // For the mock, we send directly to payment_callback
        var fd = new URLSearchParams({
            payment_id: order.payment_id,
            order_id: order.order_id,
            gateway_payment_id: mockPid,
            signature: mockSig
        });
        fd.append(csrfName, csrfToken);

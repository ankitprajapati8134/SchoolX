        ...
        <form id="onboardForm">
            ...
            <div id="step2" style="display:none;">
                <div style="padding:12px 14px;background:var(--sa-dim);border:1px solid var(--sa-ring);border-radius:8px;margin-bottom:18px;font-size:12.5px;color:var(--t2);line-height:1.7;">
                    ...
                    The first admin account is created automatically. The school admin logs in using the
                    <strong>School Code</strong> and <strong>Admin Login ID</strong> you set here.
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>School Code <span style="color:var(--rose);">*</span></label>
                            <input type="text" class="form-control" id="schoolCode" name="school_code"
                                   placeholder="e.g. SCH001" style="text-transform:uppercase;" maxlength="10" required>
                            <small id="codeAvailMsg" style="font-size:11px;display:block;margin-top:4px;"></small>
                            <small style="color:var(--t3);font-size:11px;">3–10 uppercase letters/digits. Unique across all schools.</small>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Admin Login ID <span style="color:var(--rose);">*</span></label>
                            <input type="text" class="form-control" id="adminLoginId" name="admin_login_id"
                                   placeholder="e.g. admin" required>
                            <small style="color:var(--t3);font-size:11px;">Admin enters this as their username at login.</small>
                        </div>
                    </div>
                </div>
                <div class="row">
...
$(function(){
    ...
    $('#toStep3Btn').on('click', function(){
        var code    = $('#schoolCode').val().trim().toUpperCase();
        var loginId = $('#adminLoginId').val().trim();
        var aname   = $('input[name="admin_name"]').val().trim();
        var aemail  = $('input[name="admin_email"]').val().trim();
        var pass    = $('#adminPass').val();
        if (!code)               { saToast('School code is required.','warning'); return; }
        if (!/^[A-Z0-9]{3,10}$/.test(code)) { saToast('School code must be 3–10 uppercase letters/digits.','warning'); return; }
        if (!loginId)            { saToast('Admin login ID is required.','warning'); return; }
    ...
    function buildReview(){
        ...
        html += reviewSection('ADMIN ACCOUNT',
            reviewRow('School Code',   f.find('[name=school_code]').val(), true)
            + reviewRow('Admin Login ID', f.find('[name=admin_login_id]').val(), true)
        ...
        $.ajax({
            ...
            success: function(r){
                if (r.status === 'success'){
                    ...
                    var f    = $('#onboardForm');
                    var code = f.find('[name=school_code]').val();
                    var lid  = f.find('[name=admin_login_id]').val();
                    $('#credentialsContent').html(
                        '<p style="margin-bottom:6px;"><strong>School ID:</strong> '
                        + '<code style="background:var(--bg3);padding:2px 6px;border-radius:4px;">' + code + '</code></p>'
                        + '<p style="margin-bottom:6px;"><strong>Admin ID:</strong> '
                        + '<code style="background:var(--bg3);padding:2px 6px;border-radius:4px;">' + lid + '</code></p>'

...
function loadRoleOptions(selected){
    $.post(B+'admin_users/get_roles',csrf(),function(r){
        ...
        _roles.forEach(function(rl){ opts += '<option value="'+esc(rl.role_name)+'"'+(rl.role_name===selected?' selected':'')+'>'+esc(rl.label||rl.role_name)+'</option>'; });
        $('#aRole').html(opts);
    },'json');
}
...
function renderRolesList(){
    ...
    var h = _roles.map(function(rl){
        ...
        return '<div style="padding:12px 14px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .15s;" '

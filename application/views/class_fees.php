    ...
    <div class="cf-card" id="dueFeeResult">
        ...
        <div class="cf-filter-bar">
            <div class="cf-search-wrap">
                ...
                <input type="text" id="dueSearch" class="cf-search"
                    placeholder="Search student or father name…" autocomplete="off">
            </div>
...
function loadDueFees() {
    ...
    fetch(SITE_URL + '/fees/due_fees_table', {
        ...
        body:    'class=' + encodeURIComponent(cls) + '&section=' + encodeURIComponent(sec)
    })
...
function renderTable(data) {
    ...
    data.forEach(function(rec, idx) {
        ...
        var due      = parseFloat(rec.dueFee      || 0);   // can be negative (overpaid)
        var uid      = rec.userId || '—';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (CF_INIT_CLASS) {
        // Step 1: select the class in the dropdown
        // The option values match e.g. "Class 8th" exactly
        classEl.value = CF_INIT_CLASS;
        if (!classEl.value) {
            // Try to find option that matches loosely
        ...
        if (classEl.value) {
            // Step 2: populate sections for the selected class
            populateSections(classEl.value);

            // Step 3: after a short delay, select the section and trigger load
            setTimeout(function() {
                ...
                if (CF_INIT_SECTION) {
                    ...
                    if (!matched) {
                        // If still not found, try partial match (e.g. "B" matches "Section B")
                        Array.from(sec.options).forEach(function(opt) {
                ...
                // If section is selected (by URL param or auto-select for single-section class)
...
.cf-page-title {
    ...
    font-size: 24px; font-weight: 700; color: var(--fc-navy);
    display: flex; align-items: center; gap: 10px; margin: 0 0 5px;
}
.cf-page-title > i { color: var(--fc-teal); }
.cf-breadcrumb {
    display: flex; gap: 0; list-style: none; margin: 0; padding: 0;
    font-size: 12.5px; color: var(--fc-muted);
}
.cf-breadcrumb li + li::before { content: ' / '; margin: 0 4px; color: #cbd5e1; }
.cf-breadcrumb a { color: var(--fc-teal); text-decoration: none; font-weight: 500; }
.cf-topbar-right { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
...
@media(max-width:900px) { .cf-stat-strip { grid-template-columns: repeat(2,1fr); } }
@media(max-width:500px) { .cf-stat-strip { grid-template-columns: 1fr 1fr; } }
...
.cf-stat:hover { transform: translateY(-2px); box-shadow: 0 4px 20px rgba(11,31,58,.13); }
.cf-stat-icon {
    width: 44px; height: 44px; border-radius: 10px; flex-shrink: 0;
    display: flex; align-items: center; justify-content: center; font-size: 18px;
}
.cf-stat-blue  .cf-stat-icon { background:#dbeafe; color:var(--fc-blue); }
.cf-stat-green .cf-stat-icon { background:#dcfce7; color:var(--fc-green); }
.cf-stat-amber .cf-stat-icon { background:#fef3c7; color:var(--fc-amber); }
.cf-stat-red   .cf-stat-icon { background:#fee2e2; color:var(--fc-red); }

.cf-stat-label {
    font-size: 10.5px; font-weight: 700; letter-spacing: .5px;
    text-transform: uppercase; color: var(--fc-muted); margin-bottom: 3px;
}
.cf-stat-val {
    font-family: 'Playfair Display', serif;
    font-size: 20px; font-weight: 700; color: var(--fc-navy); line-height: 1.15;
}
.cf-stat-sub { font-size: 11.5px; color: var(--fc-muted); margin-top: 2px; }
...
.cf-card-head-left { display: flex; align-items: center; gap: 9px; }
.cf-card-head-left > i { color: var(--fc-teal); }
.cf-card-head-left > h3 {
    font-family: 'Playfair Display', serif;
    font-size: 15px; font-weight: 700; color: var(--fc-navy); margin: 0;
}
.cf-card-head-right { display: flex; gap: 8px; align-items: center; }
...
.cf-field { display: flex; flex-direction: column; gap: 5px; min-width: 180px; flex: 1; }
.cf-label {
...
.cf-select {
    ...
    border: 1.5px solid var(--fc-border); border-radius: 8px;
    font-size: 13.5px; font-family: 'DM Sans', sans-serif;
    color: var(--fc-text); background: #fff; outline: none;
    appearance: none; cursor: pointer;
    transition: border-color .13s, box-shadow .13s;
}
.cf-select:focus { border-color: var(--fc-teal); box-shadow: 0 0 0 3px rgba(14,116,144,.1); }
.cf-select-arr {
    position: absolute; right: 11px; top: 50%; transform: translateY(-50%);
    color: var(--fc-muted); font-size: 11px; pointer-events: none;
}
...
.cf-btn:hover:not(:disabled) { opacity: .85; transform: translateY(-1px); }
.cf-btn:disabled { opacity: .38; cursor: not-allowed; transform: none !important; }
.cf-btn-sm { padding: 6px 13px; font-size: 12px; height: 34px; }
.cf-btn-primary { background: var(--fc-teal); color: #fff; box-shadow: 0 2px 8px rgba(14,116,144,.3); }
.cf-btn-navy    { background: var(--fc-navy); color: #fff; }
.cf-btn-ghost   { background: var(--fc-white); color: var(--fc-text); border: 1.5px solid var(--fc-border); }
.cf-btn-ghost:hover:not(:disabled) { border-color: var(--fc-teal); color: var(--fc-teal); opacity: 1; }
.cf-btn-amber   { background: var(--fc-amber); color: #fff; }

/* ── Results area ── */
#dueFeeResult { display: none; }
...
.cf-search-wrap { position: relative; flex: 1; max-width: 280px; }
.cf-search-icon {
    position: absolute; left: 10px; top: 50%; transform: translateY(-50%);
    color: var(--fc-muted); font-size: 12px; pointer-events: none;
}
.cf-search {
    width: 100%; height: 36px; padding: 0 10px 0 30px;
    border: 1.5px solid var(--fc-border); border-radius: 8px;
    font-size: 13px; font-family: 'DM Sans', sans-serif;
    outline: none; background: #fff; color: var(--fc-text);
    transition: border-color .13s, box-shadow .13s;
}
.cf-search:focus { border-color: var(--fc-teal); box-shadow: 0 0 0 3px rgba(14,116,144,.1); }
.cf-row-count { font-size: 12px; color: var(--fc-muted); font-weight: 600; white-space: nowrap; }

/* Due table */
.cf-table-wrap { overflow-x: auto; max-height: 520px; overflow-y: auto; }
.cf-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.cf-table thead th {
    background: var(--fc-navy); color: rgba(255,255,255,.88);
    padding: 10px 14px; font-size: 10.5px; font-weight: 700;
    letter-spacing: .4px; text-transform: uppercase;
    white-space: nowrap; text-align: left;
    position: sticky; top: 0; z-index: 4;
}
.cf-table thead th.r { text-align: right; }
.cf-table thead th.c { text-align: center; }
.cf-table thead th:first-child { padding-left: 18px; }

.cf-table td {
    padding: 10px 14px; border-bottom: 1px solid var(--fc-border);
    vertical-align: middle; font-size: 13px;
}
.cf-table td:first-child { padding-left: 18px; }
.cf-table td.r { text-align: right; font-variant-numeric: tabular-nums; }
.cf-table td.c { text-align: center; }

.cf-table tbody tr { cursor: pointer; transition: background .1s; }
.cf-table tbody tr:hover { background: var(--fc-sky); }
.cf-table tbody tr.cf-selected { background: #bee3ff !important; }

/* Name cell */
.cf-name-cell { display: flex; align-items: center; gap: 9px; }
.cf-avatar {
...
.cf-name-text { line-height: 1.3; }
.cf-student-name { font-weight: 600; color: var(--fc-text); display: block; }
.cf-father-name  { font-size: 11.5px; color: var(--fc-muted); }

/* Amount cells */
.cf-amt-total    { font-weight: 600; }
.cf-amt-received { font-weight: 600; color: var(--fc-green); }
.cf-amt-due      { font-weight: 700; }
.cf-amt-due.over  { color: var(--fc-green); }
.cf-amt-due.exact { color: var(--fc-muted); }
.cf-amt-due.due   { color: var(--fc-red); }
...
.cf-badge-paid    { background: #dcfce7; color: #14532d; }
.cf-badge-partial { background: #fef3c7; color: #92400e; }
.cf-badge-due     { background: #fee2e2; color: #991b1b; }
.cf-badge-over    { background: #dbeafe; color: #1e40af; }

/* Progress bar */
.cf-progress-wrap { width: 100%; min-width: 80px; }
.cf-progress-bg {
    width: 100%; height: 6px; background: var(--fc-border);
...
.cf-progress-fill.paid    { background: var(--fc-green); }
.cf-progress-fill.partial { background: var(--fc-amber); }
.cf-progress-fill.due     { background: var(--fc-red); }
.cf-progress-fill.over    { background: var(--fc-blue); }
.cf-progress-pct { font-size: 10.5px; color: var(--fc-muted); margin-top: 2px; text-align: right; }

/* Tfoot */
.cf-table tfoot td {
    background: var(--fc-navy); color: rgba(255,255,255,.9);
    font-weight: 700; font-size: 12.5px; padding: 10px 14px;
    text-align: right; position: sticky; bottom: 0; z-index: 3;
}
.cf-table tfoot td:first-child { text-align: left; padding-left: 18px; }
.cf-table tfoot .cf-foot-green  { color: #6ee7b7; }
.cf-table tfoot .cf-foot-red    { color: #fca5a5; }

/* Loading / empty */
.cf-loader {
    text-align: center; padding: 40px 20px; color: var(--fc-muted);
}
.cf-loader i    { font-size: 24px; }
.cf-loader p    { margin: 10px 0 0; font-size: 14px; }
.cf-empty-res   { text-align: center; padding: 50px 20px; color: var(--fc-muted); }
.cf-empty-res i { font-size: 40px; opacity: .2; display: block; margin-bottom: 12px; }
.cf-empty-res p { margin: 0; font-size: 14px; }
...
.cf-toast {
    ...
    animation: cfSlideIn .25s ease; pointer-events: auto;
    max-width: 320px;
}
.cf-toast-success { border-left: 4px solid var(--fc-green); }
.cf-toast-error   { border-left: 4px solid var(--fc-red); }
.cf-toast-warning { border-left: 4px solid var(--fc-amber); }
.cf-toast-info    { border-left: 4px solid var(--fc-teal); }
.cf-toast-hide    { opacity: 0; transition: opacity .3s; }
@keyframes cfSlideIn { from { transform: translateX(30px); opacity: 0; } to { transform: none; opacity: 1; } }
</style>

...
class Admission_public extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->firebase = $this->firebase ?? null;
    }
    ...
    public function submit($school_id = '')
    {
        ...
        $leadId = 'PUB_' . time() . '_' . mt_rand(1000, 9999);
        $crmBase = "Schools/{$school_id}/CRM/Admissions";

        // Read counter for sequential application ID
        $counterPath = "{$crmBase}/Counter";
        $counter = (int) ($this->firebase->get($counterPath) ?? 0);
        $counter++;
        $appId = 'APP' . str_pad($counter, 4, '0', STR_PAD_LEFT);
    ...
    public function payment_callback($school_id = '')
    {
        ...
        if ($verified) {
            ...
            notify_sms($payment['phone'] ?? '', "Payment of INR {$payment['amount']} received for {$schoolName} admission (Ref: {$paymentId}). Your application is now approved.", [
            ...
            $this->_public_audit($school_id, 'Payment', 'payment_success', $paymentId,
                "Admission fee paid: INR {$payment['amount']} for app {$appId}");
